...
function getloginattempt($type, $connection)
{
    ...
    if ($type == "GET") {
        ...
        $getipresult = $connection->query($getip);
        ...
        if ($getipresult->num_rows > 0) {
            ...
            //echo $ip, $time;
            if ($connection->query($insertattempt) === TRUE) {
                return 0;
            }
        }
    } else if ($type == "FAIL") {
        ...
        if ($getipresult->num_rows > 0) {
            ...
            if ($row["Attempt"] < 2) {
            	...
            	addattempttime($row["IP"], $row["Attempt"], "0S", $connection);
            } elseif ($row["Attempt"] < 4) {
            	addattempttime($row["IP"], $row["Attempt"], "1M", $connection);
            } else {
            	addattempttime($row["IP"], $row["Attempt"], "5M", $connection);
            }
        }
    } else if ($type == "SUCCESS") {
        $deleteattempt = "DELETE FROM loginattempt WHERE IP = '$ip'";
        // "UPDATE loginattempt SET attempt= 0 WHERE ip= '$ip'";
        $rundeleteattempt = mysqli_query($connection, $deleteattempt);
    }
}
...
function addattempttime($ip, $attempt, $waiting, $connection) {
	$time = new DateTime();
    ...
    $runinsertattempt = mysqli_query($connection, $insertattempt);
}
